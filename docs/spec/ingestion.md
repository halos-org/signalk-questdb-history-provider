...
| Delta fields read                    | `path`, `value`, `context`, `$source`, `isMeta`                                                 | No other delta field is read                                                                      |
| Meta delta test                      | `isMeta === true`                                                                               | Strict equality; the server marks a meta update this way                                          |
...
| Track path                           | `navigation.position`                                                                           | The only path whose object values can produce a track row; its other object values produce no row |
...
| Leaf path form                       | `<parent path>#/<escaped key>`                                                                  | Path of a leaf produced from an object value, an RFC 6901 pointer after `#`                       |
| Key escaping                         | `~` becomes `~0`, then `/` becomes `~1`                                                         | RFC 6901 order; no other character is altered                                                     |
...
3. The plugin reads `path`, `value`, `context`, `$source` and `isMeta` from the delta. A delta whose `$source` is a non-empty string yields rows carrying that source; a delta whose `$source` is absent, not a string, or the empty string yields rows with no source. The plugin never reads `timestamp`, `source` or `state`.
...
1. Discard a meta delta: one whose `isMeta` is `true`. Meta updates (`units`, `description`, `displayName` and the like) describe a path; they are not history. Any other `isMeta` value, or none, is not a meta delta.
2. Vessel identity check (see "Vessel identity deltas"). An identity delta never continues to the data steps, whether it produces a row or not.
3. Discard when `path` is empty or missing, or when `value` is `null` or `undefined`. `false`, `0` and `""` are values and are not discarded here.
4. Determine whether the delta is from the own vessel (`context === app.selfContext`). Discard an own-vessel delta when `recordSelf` is `false`. Discard any other delta when `recordOthers` is `false`. "Other" covers every non-self context, including `atons.*`, `aircraft.*` and `shore.*` contexts. A delta whose `context` is undefined or not a string is not the own vessel and is not discarded here (see "Observed defects").
5. Determine the value kind (see "Routing by kind"):
   ...
   - any other object at path exactly `navigation.position` is a half position and is discarded;
...
6. Apply the path filter to `path`; discard when the filter rejects it. Then apply the sampling gate to (`path`, stored context); discard when the gate rejects it. For an object value, `path` is the object's path and both gates are applied once for the whole delta, never per leaf. An object with no usable leaf is discarded before this step and opens no sampling window.
7. Compute the stored context: `self` for the own vessel, the delta's `context` verbatim otherwise.
8. The delta produces the rows for its kind, in the table and form given under "Routing by kind", with the columns given under "Columns common to every row form". No row carries the delta's timestamp.
...
The rules in this section apply to a value that has passed steps 1 to 7 of "Order of decisions for one delta", and to each leaf of an object value whose delta has passed them (see "Object values: one-level flattening"). A value under the empty path never reaches these rules; it is either an identity delta or discarded.
...
13. A value under `navigation.position` that is an object but fails rule 10 (a coordinate missing, non-numeric or non-finite) is a half position and produces no row in any table. It is not flattened: a pointer leaf under the position path would stand beside the track rows as a second, partial encoding of the same path. It is discarded before the filter and the sampling gate, so it opens no sampling window. A value under `navigation.position` that is null produces no row.
...
1. A value that is an object, not null, not an array, and not under `navigation.position`, produces zero or more rows, one per scalar leaf. No row is produced for the object's own path in any table.
2. The leaf path is the object's path, `#/`, and the key escaped per RFC 6901: each `~` becomes `~0`, then each `/` becomes `~1`. The object `{roll: 0.02}` under `navigation.attitude` produces a row with path `navigation.attitude#/roll`; the key `a/b` becomes `a~1b` and the key `a~b` becomes `a~0b`. A Signal K path never contains `#`, so a leaf path cannot be mistaken for a scalar path.
...
6. A leaf that is a non-finite number, a nested object, an array, null, undefined, or any other kind produces no row. Nested objects are not descended into: flattening is one level deep, and a payload such as a resource document loses everything below its first level.
7. A leaf whose key is the empty string produces no row.
8. An object with no usable leaf (an empty object, or one whose every leaf falls under rule 6 or 7) produces no row and is not gated, so it opens no sampling window.
9. Leaves are taken in the order the object's own enumerable string keys enumerate, and rows are written in that order.
...
11. The delta is gated once, on the object's path: the path filter is applied to the object's path, then the sampling gate is applied to (object path, stored context) with the rate resolved for the object's path. When either rejects, no leaf is recorded; when both admit, every usable leaf is recorded. A delta never stores some of its fields and not others.
12. Because only the object's path is consulted, an exclude-mode filter listing the literal `navigation.attitude` records none of its leaves, an include-mode filter listing only `navigation.attitude.roll` records nothing for attitude, and a sampling rate for `navigation.attitude` throttles all of its leaves together. Entries naming a dotted field (`navigation.attitude.roll`) or a glob that matches only fields (`navigation.attitude.*`) do not apply to the object.
13. The object's sampling window is the (object path, stored context) window, shared with a scalar delta at the same path and stored context.
14. All leaves of one delta carry the same `ts` (see "Columns common to every row form"). Their rows stay distinct under the deduplication keys because their `path` differs.
15. An object under the empty path is never flattened (it is an identity delta or is discarded). No row is ever written under a path that begins with `#`.

### Columns common to every row form

1. `ts` is the server receive time, assigned once per delta when its rows are formed, not the timestamp the delta carries. Every row of one delta carries the same `ts`; rows of two deltas formed in the same millisecond receive distinct `ts` values at microsecond resolution. The ILP wire format surface specifies the clock. Because sampling windows also measure the server clock, a source whose own clock is wrong is recorded and sampled at the configured rate regardless.
...
- Timestamp: assigned by the ILP wire format surface as the current server time, once per delta, never the delta's `timestamp`; shared by the rows of one delta and distinct across deltas at microsecond resolution.
- Leaf paths `<path>#/<escaped key>` are written by this surface; the history surfaces read them.
...
| Path `navigation.position`, value `{latitude: 1}`                                                                                               | Delta arrives      | No row in any table.                                                                                                                                                  |
...
| Path `navigation.position`, value `{latitude: NaN, longitude: 13.4}`                                                                            | Delta arrives      | No row in any table.                                                                                                                                                  |
| Path `navigation.position`, value `{latitude: "52.5", longitude: 13.4}`                                                                         | Delta arrives      | No row in any table.                                                                                                                                                  |
| Path `navigation.position`, value `{latitude: 52.5, longitude: Infinity}`                                                                       | Delta arrives      | No row in any table.                                                                                                                                                  |
| Path `navigation.position`, value `{latitude: 52.5}`, then `{latitude: 52.5, longitude: 13.4}` from the same context inside the sampling window | Both deltas arrive | One row in `signalk_position`; the half position opened no sampling window.                                                                                           |
...
| Input / state                                                                                               | Action        | Expected outcome                                          |
| ----------------------------------------------------------------------------------------------------------- | ------------- | --------------------------------------------------------- |
| Path `navigation.speedOverGround`, value `{units: "m/s", description: "Speed over ground"}`, `isMeta: true` | Delta arrives | No row in any table.                                      |
| Path `navigation.speedOverGround`, value `6.4`, `isMeta: true`                                              | Delta arrives | No row in any table.                                      |
| Path `navigation.speedOverGround`, value `6.4`, `isMeta: false`                                             | Delta arrives | One row in `signalk`: `navigation.speedOverGround` `6.4`. |

### Flattening objects

| Input / state                                                                                                                           | Action                                   | Expected outcome                                                                                                                                                                                                                              |
| --------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Path `navigation.attitude`, value `{roll: 0.02, pitch: -0.01, yaw: 1.57}`                                                               | Delta arrives                            | Three rows in `signalk`, in this order: `navigation.attitude#/roll` `0.02`, `navigation.attitude#/pitch` `-0.01`, `navigation.attitude#/yaw` `1.57`, all handed over as one delta. No row under `navigation.attitude`.                        |
...
| Path `a.b`, value `{"a/b": 1, "a~b": 2, "": 3}`                                                                                         | Delta arrives                            | Two rows in `signalk`: `a.b#/a~1b` `1`, `a.b#/a~0b` `2`. No row for the empty key.                                                                                                                                                            |
| Path `a.b`, value `{}`                                                                                                                  | Delta arrives                            | No row in any table.                                                                                                                                                                                                                          |
| Path `a.b`, value `{}`, then `{x: 1}` from the same context inside the sampling window                                                  | Both deltas arrive                       | One row: `a.b#/x` `1`; the empty object opened no sampling window.                                                                                                                                                                            |
...
| Mode `exclude`, patterns `["navigation.attitude"]`; path `navigation.attitude`, value `{roll: 0.02, pitch: -0.01, yaw: 1.57}`           | Delta arrives                            | No row in any table.                                                                                                                                                                                                                          |
| Mode `include`, patterns `["navigation.attitude.roll"]`; path `navigation.attitude`, value `{roll: 0.02, pitch: -0.01}`                 | Delta arrives                            | No row in any table.                                                                                                                                                                                                                          |
| Rates `{"navigation.attitude": 1000}`, default `0`; path `navigation.attitude`, value `{roll, pitch, yaw}` at `T0`, `T0+500`, `T0+1000` | Deltas arrive                            | Six rows: all three leaves at `T0` and all three at `T0+1000`; none at `T0+500`.                                                                                                                                                              |
| A scalar delta at `navigation.attitude` from `self` at `T0`; rate `2000`                                                                | An attitude object from `self` at `T0+1` | No row for the object; the scalar and the object share the `navigation.attitude` window.                                                                                                                                                      |
